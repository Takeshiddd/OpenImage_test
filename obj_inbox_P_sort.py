    ...
    for classbbox in Image_dict[ImageID]:
        ...
        for textbbox in text_dict[ImageID]:
            if inbox(classbbox['position'], textbbox['position']):
                c += 1
        if c != 0:
            counter[classbbox['classname'].lower()] += 1

with open('object_inbox_num.csv', 'w') as file:
    writer = csv.writer(file, lineterminator='\n')
    for k, v in counter.items():
        writer.writerow([k, v])

obj_count_dict = defaultdict(float)
with open('object_rank_train.csv', 'r') as f:
   reader = csv.reader(f)
   for row in reader:
      obj_count_dict[row[0]] += float(row[1])

P_list = []
for key in obj_count_dict.keys():
   if key in counter.keys():
       if obj_count_dict[key] > 10:
            P_list.append([key, counter[key] / obj_count_dict[key]])
P_list.sort(key=itemgetter(1), reverse=True)

with open('inbox_P_sort.csv', 'w') as file:
    writer = csv.writer(file, lineterminator='\n')
